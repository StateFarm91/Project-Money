...
6. **Track B product research now** (no credentials needed): Meta Ad Library long-running ads → CJ US-warehouse availability → margin and legitimacy filter → 3 products + brand name + domain proposal (`products/store/PRODUCT_RESEARCH.md`); then the Shopify/CJ/Meta clients and creative pipeline as Block 2 parts land.
