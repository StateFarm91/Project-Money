...
7. Meanwhile for Track A (no credentials needed): all five listing generators (incl. the bundle) are now done, and the Printify API client (`products/pod/printify_api.py`) is now written and unit-tested. Remaining: ledger importer for Etsy Payments; `ops/status.py` Etsy extension; 3-5 Canadian-gift designs (mugs/prints/totes, C19) rendered programmatically for the POD line (publish once `PRINTIFY_TOKEN` exists).
