"""E4 assets: as E3's, from the same frozen geometry, with the yarn continuous (the masks
now carry the whole yarn) and the reference being the presentation3 render.
    ...
    for kind in kinds:
        ...
        for view in views:
            ...
            manifest[kind]["views"][view] = {
                ...
                "reference_rgb": os.path.join(D, f"{kind}_draped_presentation3_{view}.png"),
                "reference_rgb_sha256": (hashlib.sha256(open(os.path.join(D, f"{kind}_draped_presentation3_{view}.png"), "rb").read()).hexdigest() if os.path.exists(os.path.join(D, f"{kind}_draped_presentation3_{view}.png")) else None)}
