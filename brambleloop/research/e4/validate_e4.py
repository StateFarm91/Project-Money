...
def aligned_masks(kind, view, gen_path):
    ...
    s = np.sqrt(len(rys) / max(len(ys), 1)); z = zoom(gmask.astype(float), s, order=1) > 0.5
...
def global_props(kind, view, gen_path):
    ...
    a, b = struct(ref_rgb), struct(gen); b_al = zoom(b, align["scale"], order=1)
    canvas = np.zeros_like(a); h, w = b_al.shape; sy0, sx0 = int(round(align["shift_px"][0])), int(round(align["shift_px"][1]))
    ...
    return {"status": "FAIL" if "FAIL" in statuses else "PASS", "items": {k: {"status": s, "evidence": e} for k, (s, e) in items.items()}}
...
def main(tag="e4"):
    ...
    for run in man["runs"]:
        ...
        r = {"kind": kind, "view": view, "generated": run["path"], "output_sha256": run["output_sha256"], "geometry_sha256": run["geometry_sha256"],
        ...
        json.dump(j, open(os.path.join(OUT, f"judge_{kind}_{view}_{tag}.json"), "w"), indent=1)
        results.append(r)
        print(f"{kind} {view}: structural {structural} | local {loc['status']} ({loc.get('tested')} tested, failed {loc.get('failed_stitches')}, mean IoU {loc.get('mean_iou') and round(loc['mean_iou'], 3)}) | global {glob['status']} { {k: v['status'] for k, v in glob['items'].items()} }")
