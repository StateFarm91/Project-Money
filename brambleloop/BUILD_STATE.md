...
Where the 44 executable requirements now wait:

| gate | executable | what opens it | cost |
|---|---|---|---|
| `rendered_pages` | 20 | a browser worker that actually fetched an Etsy page | see below |
| `image_generation` | 12 | one image actually generated by a reference-conditioning provider | ~CA$3.40 first month |
| `benchmark_purchases` | 4 | one purchased benchmark pattern row | owner purchase |
...
Measured by `./run_tests.sh` on commit `PLACEHOLDER`: **2,637 tests passing, 0
