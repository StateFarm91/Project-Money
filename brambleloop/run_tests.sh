...
for t in tests/test_compiler.py tests/test_reverse.py tests/test_twin.py \
         tests/test_platform.py tests/test_gates.py tests/test_radar.py tests/test_gateway.py tests/test_brand.py tests/test_shadow.py \
         tests/test_persistence.py tests/test_chaos.py tests/test_deploy.py tests/test_product_run.py; do
  echo "== $t"
  out=$($PY "$t" 2>&1); code=$?
  echo "$out" | sed 's/^/   /'
