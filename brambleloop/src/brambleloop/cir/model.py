...
class Material:
    ...
    **It defaults to empty and an empty value means "not stated", never "no fibre
    ...
    def __post_init__(self) -> None:
        ...
        # fact having one representation: `CIR.compile_key` hashes `to_dict`, so declaration
        # order would give the same yarn two compile keys and re-run certification for
        # nothing; and descending order by weight is how a composition is customarily
