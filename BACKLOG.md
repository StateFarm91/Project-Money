...
- [x] Physical line via print-on-demand (C19): ~~Printify API client~~ done (`products/pod/printify_api.py`, unit-tested, no live calls yet), Canadian print providers filter built in; owner creates a Printify account (10 min); zero cost until an order (D-007). Still open: 3-5 Canadian-gift designs (mugs/prints/totes) rendered programmatically, mockups, listing copy.
