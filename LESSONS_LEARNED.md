...
### LL-010: Text rendered at a fixed font size will silently run off the canvas once real (unpredictable) input arrives
- Building the POD line's personalized designs (`products/pod/build_designs.py`), the first draft used a fixed font size for buyer-personalizable text (town name, surname, hockey-family name/role). It looked fine with short sample data ("Muskoka", "McKenna", "James") but a deliberate stress test with longer, realistic inputs ("Sault Ste. Marie", "Papineau-Beauchamp", "grandmother") ran text off both edges of the canvas — silently producing a broken print file, no exception, no visual cue except opening the image. Fix: a `fit_font()` helper that shrinks the font until the text fits a safe width, applied to every personalizable string, plus an automated test (`test_build_designs.py`) that renders long *and* short sample text and asserts no ink reaches the outer margin. Reusable rule: whenever a design/template renders text whose length isn't fixed at build time (buyer input, API data, user-generated content), test it with deliberately long and short samples before calling it done — the happy-path default is exactly where this bug hides.
